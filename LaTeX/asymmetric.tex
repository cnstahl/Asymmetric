...
\charlie{I'm going to introduce staircase circuits first and then the $\Gamma(s)$ formalism, but I don't know if I should do it the other way around.}
...
For large $n$, approaching the size of the system, we can approximate the entanglement curve as being uncorrelated. In that limit, the growth rate is $\Gamma(ds/dt) = ds/dt+1$, so that for spreading to the left $v_B=1$ and for spreading to the right $v_B=\infty$. This is of course maximally asymmetric. 
