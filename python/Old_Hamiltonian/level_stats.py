...
for i in range(L):
    ...
    Y += Yi
S2 = np.matmul(X,X) + np.matmul(Y,Y) + np.matmul(Z,Z)
...
Sdiag = np.matmul(np.matmul(vecsS.conj().T, Hsmal), vecsS)
vals = np.round(valsS).astype('int')
counts = np.bincount(vals)
mask = (vals == np.argmax(counts))

Htiny = np.matmul(np.matmul(vecsS.conj().T, Hsmal), vecsS)[mask][:,mask]
